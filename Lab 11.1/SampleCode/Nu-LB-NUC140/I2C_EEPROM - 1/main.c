...
int32_t main(void)
{
  ...
  //sprintf(Text,"EEPRON begin"); // print scankey input to string
  //print_Line(0, Text);         // display string on LCD
  ...
  while (1)
  {
    ...
    sprintf(Text, "Send"); // print scankey input to string
    print_Line(0, Text);   // display string on LCD

    sprintf(Text, "%d", sum); // print scankey input to string
  ...
  password[3] = sum % 10;
  password[2] = (sum % 100) / 10;
  password[1] = (sum % 1000) / 100;
  password[0] = (sum % 10000) / 1000;

  //clear_LCD();
  /*
	while(1){
		for(i=0; i<4;i++){
			sprintf(Text,"%d",password[i]); // print scankey input to string		
			print_Line(i, Text);         // display string on LCD
		}
	}*/
  //clear_LCD();
  Init_I2C();
  ...
  {
    data = EEPROM_Read(i); // read EEPROM address= i
    sum_out = sum_out * 10 + data;
  }
  sprintf(Text, "%d", sum_out); // print scankey input to string
  print_Line(1, Text);          // display string on LCD
  sprintf(Text, "send");        // print scankey input to string
  printS(80, 16 * 1, Text);
